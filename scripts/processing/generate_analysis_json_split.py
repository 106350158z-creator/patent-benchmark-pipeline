...
SOURCE_RELIABILITY_RULES = """Critical source reliability rules:
- SOURCE must contain actual examination/prosecution material before you infer grantability dimensions.
- Publication text, patent specifications, claims, descriptions, examples, and original application PDFs are not evidence that an examiner accepted novelty, inventive step, sufficiency, clarity, or eligibility.
- Absence of an objection in SOURCE is never positive evidence. Do not give a high score merely because no challenge is shown.
- Give 100 only when examination/prosecution material directly supports that the dimension passed or an objection was overcome.
- If SOURCE lacks direct examination evidence for this dimension, use score 50, state that the source is insufficient, and cite the best continuous SOURCE passage showing what material was actually available.
"""
...
def main() -> None:
    ...
    parser.add_argument("--skip-meta", action="store_true", help="Skip the meta extraction call; useful when rerunning selected dimensions for an existing analysis.")
    ...
    prompts = {} if args.skip_meta else {"meta": build_meta_prompt(compact, sources)}
    ...
    if args.write_steps and not args.skip_meta:
