...
def source_has_examination_material(source_texts: list[dict[str, str]]) -> bool:
    combined = "\n".join(f"{doc.get('name', '')}\n{doc.get('text', '')}" for doc in source_texts).lower()
    return any(marker in combined for marker in EXAMINATION_SOURCE_MARKERS)


def source_looks_publication_only(source_texts: list[dict[str, str]]) -> bool:
    if not source_texts or source_has_examination_material(source_texts):
        return False
    combined = "\n".join(f"{doc.get('name', '')}\n{doc.get('text', '')}" for doc in source_texts).lower()
    publication_hits = sum(1 for marker in PUBLICATION_ONLY_MARKERS if marker in combined)
    names = " ".join(str(doc.get("name") or "").lower() for doc in source_texts)
    generated_claim_docs = bool(re.search(r"(^|\s|/|\\)[a-z0-9]+_claims(?:_\d+)?\.txt\b", names))
    return publication_hits >= 2 or generated_claim_docs
...
def benchmark_quality_issues(benchmark: dict[str, Any], source_texts: list[dict[str, str]]) -> list[str]:
    ...
    if source_texts and not source_has_examination_material(source_texts):
        issues.append("selected SOURCE documents do not appear to contain EPO examination/prosecution material")
    if source_looks_publication_only(source_texts):
        issues.append("selected SOURCE documents look like publication/specification text only, not examination material")
    return issues
