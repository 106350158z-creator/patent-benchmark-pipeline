...
def fetch_doclist(
    ...
    browser_manual_wait_seconds: int,
) -> Path:
    ...
    if proxy_url:
        args += ["-ProxyUrl", proxy_url]
    if use_cached:
        args.append("-UseCached")
    try:
        run_kwargs: dict[str, Any] = {"cwd": str(project_root), "check": True}
        if browser_fallback:
            run_kwargs.update({"stdout": subprocess.PIPE, "stderr": subprocess.PIPE, "text": True, "encoding": "utf-8", "errors": "replace"})
        ...
        browser_args = [
            sys.executable,
            "scripts\\fetch_epo_doclist_browser.py",
            "--application-number",
            app,
            "--output-dir",
            str(app_cache),
            ...
            browser_profile_dir,
            "--manual-wait-seconds",
            str(browser_manual_wait_seconds),
        ]
        if browser_proxy_server:
            browser_args += ["--proxy-server", browser_proxy_server]
        if browser_headless:
            browser_args.append("--headless")
        if use_cached:
            browser_args.append("--use-cached")
        with BROWSER_FALLBACK_LOCK:
            subprocess.run(browser_args, cwd=str(project_root), check=True)
...
def validate_one(
    ...
    try:
        csv_path = fetch_doclist(
            ...
            args.use_cached,
            args.epo_proxy_url,
            args.browser_doclist_fallback,
            args.browser_profile_dir,
            args.browser_proxy_server or args.epo_proxy_url,
            args.browser_headless,
            args.browser_manual_wait_seconds,
        )
...
def main() -> None:
    ...
    parser.add_argument("--epo-proxy-url", default="", help="Explicit proxy URL for EPO Register doclist validation, for example http://127.0.0.1:7897.")
    parser.add_argument("--browser-doclist-fallback", action="store_true", help="If the PowerShell doclist fetch fails, retry through a persistent Playwright browser session.")
    parser.add_argument("--browser-profile-dir", default="markush-run/_state/epo-register-browser-profile")
    parser.add_argument("--browser-proxy-server", default="", help="Proxy server for browser fallback. Defaults to --epo-proxy-url when omitted.")
    parser.add_argument("--browser-headless", action="store_true", help="Run browser fallback headless. Headed mode is safer for Cloudflare clearance reuse.")
